...
class RenewalExecutor {
    ...
    async findAndClickRenewalButton() {
        ...
        try {
            ...
            if (renewForm) {
                ...
                buttonFound = true;
                logger_1.logger.info('RenewalExecutor', '已通过表单提交续期请求');
            }
        }
        catch (error) {
            logger_1.logger.warn('RenewalExecutor', '表单提交失败,尝试其他方式', error);
        }
            ...
            for (const selector of renewalButtonSelectors) {
                try {
                    ...
                    if (button) {
                        await button.click();
                        buttonFound = true;
                        logger_1.logger.info('RenewalExecutor', `已点击续期按钮: ${selector}`);
                        break;
                    }
                }
                catch (error) {
        ...
        if (!buttonFound) {
            ...
            if (found) {
                buttonFound = true;
                logger_1.logger.info('RenewalExecutor', '已通过文本内容点击续期按钮');
            }
        }
        if (!buttonFound) {
            throw new types_1.RenewalError(types_1.ErrorType.PARSE_ERROR, '未找到续期按钮,可能页面结构已变化');
        }
        // 等待一下,让页面响应
        await delay(1000);
    }
    ...
    async handleRenewalConfirmation() {
        logger_1.logger.info('RenewalExecutor', '等待续期确认对话框...');
        try {
            // 等待可能出现的确认对话框
            await delay(2000);
            // 检查是否有确认按钮
            ...
            for (const selector of confirmButtonSelectors) {
                try {
                    ...
                    if (button && await button.isIntersectingViewport()) {
                        ...
                        logger_1.logger.info('RenewalExecutor', '已点击确认按钮');
                        break;
                    }
                }
                catch (error) {
            ...
            if (!confirmationHandled) {
                logger_1.logger.info('RenewalExecutor', '未找到确认对话框,可能直接进入处理流程');
            }
            await delay(1000);
        }
        catch (error) {
            logger_1.logger.warn('RenewalExecutor', '处理确认对话框时出错,可能不需要确认', error);
        }
    }
    /**
     * 等待续期处理完成
